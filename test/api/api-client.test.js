...
describe("fetchWordSuggestions", () => {
    it("should throw an error if response is not ok", async () => {
        ...
        await expect(fetchWordSuggestions("query", false)).rejects.toThrow("HTTP Status: 400");
    });
    ...
    it("should log an error on fetch failure", async () => {
        ...
        await expect(fetchWordSuggestions("query", false)).rejects.toThrow("HTTP Status: 404");
        expect(consoleSpy).toHaveBeenCalledWith(`Failed to fetch data for suggestion: query`, expect.any(Error));
