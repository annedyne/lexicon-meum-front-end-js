...
describe("handleLoadWordDetail", () => {
    ...
    it("throws loading error on fetch error", async () => {
        ...
        await expect(handleLoadWordDetail("example", lexemeId)).rejects.toThrow(
            "There was a problem loading details for example.",
        );
    ...
    it("throws displaying error on non-fetch error", async () => {
        ...
        const lexemeId = 456;
        await expect(handleLoadWordDetail("test", lexemeId)).rejects.toThrow(
            "There was a problem displaying details for test.",
        );
        expect(renderWordDetail).not.toHaveBeenCalled();
    });
});
